...
function bootstrap() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (let file in files) {
            ...
            if (extension === "dwg") {
                const opts = new pdfnet_node_1.PDFNet.Convert.CADConvertOptions();
                opts.setPageWidth(800);
                opts.setPageHeight(600);
                opts.setRasterDPI(150);
                console.log(`adding ${files[file]}`);
                yield pdfnet_node_1.PDFNet.Convert.fromCAD(doc, path_1.default.join(directoryPath, files[file]), opts);
            }
        }
        try {
            doc.save(`${name}.pdf`, pdfnet_node_1.PDFNet.SDFDoc.SaveOptions.e_linearized);
        }
        catch (e) {
            console.error(e);
        }
